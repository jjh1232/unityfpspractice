...
public class ReptileCtrl : MonoBehaviour {
    ...
    public float attackDist = 8.0f; // 공격 범위
    public int HP = 2;
    public bool Look = false;
    public bool Reptile = true;
    ...
    IEnumerator CheckState()
    {
        while (!isDead)
        {
            ...
            if (dist <= attackDist && !EatGround.safe)
            {
                curState = CurrentState.attack;
            }
            else if (dist <= traceDist && !EatGround.safe)
            {
